...
const JoinRoomForm = () => {
  const [formData, setFormData] = useState<JoinRoomFormData>(InitialJoinRoomFormData);
  ...
  const { execute: joinRoomExecute, loading: joining } = usePublicAction(playerApi.join, {
    onSuccess(data) {
      ...
      <Button
        onClick={() => {
          if (!formData.roomCode) {
            toast.error("Enter a room code", { className: josephinBold.className });
            return;
          }
          checkExistingPlayer();
        }}
  ...
  const PlayerDetails = () => (
    <>
      ...
      <Button onClick={joinRoom} className="mt-4" disabled={joining}>Join Room</Button>
    </>
  );
